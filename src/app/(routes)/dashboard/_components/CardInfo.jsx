...
import aiFinanceData from "../../../../../utils/aiFinanceData";

function CardInfo({budgetList, incomeList}) {
    const [totalBudget, setTotalBudget] = useState(0);
    const [totalSpend, setTotalSpend] = useState(0);
    const [totalIncome, setTotalIncome] = useState(0);
    const [financecialAdvice, setFinancecialAdvice] = useState("");
    ...
    const CalculateCardInfo = () => {
        ...
        incomeList.forEach((income) => {
            totalIncome_ += Number(income.amount) || 0;
        });
    ...
    useEffect(() => {
      if (totalBudget > 0 || totalIncome > 0 || totalSpend > 0) {
        const fetchFinancialAdvice = async () => {
          const advice = await aiFinanceData(totalBudget, totalIncome, totalSpend);
                ...
                <div>
                    <div className="p-7 border mt-4 rounded-2xl flex items-center justify-between">
                       <div className="">
                            ...
                            <h2 className="font-light text-md">
                                {financecialAdvice || "Your financial advice will be here soon!"}
                            </h2>
                       </div>
                    </div>
                    <div className="mt-7 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                      <div className="p-7 border rounded-2xl flex items-center justify-between">
                        ...
                        <PiggyBank className="bg-teal-800 p-3 h-12 w-12 rounded-full text-white" />
                      </div>
                      ...
                      <div className="p-7 border rounded-2xl flex items-center justify-between">
                        ...
                        <ReceiptText className="bg-teal-800 p-3 h-12 w-12 rounded-full text-white" />
                      </div>
                      ...
                      <div className="p-7 border rounded-2xl flex items-center justify-between">
                        ...
                        <Wallet className="bg-teal-800 p-3 h-12 w-12 rounded-full text-white" />
                      </div>
                      ...
                      <div className="p-7 border rounded-2xl flex items-center justify-between">
                        ...
                        <BadgeIndianRupee className="bg-teal-800 p-3 h-12 w-12 rounded-full text-white" />
                      </div>
                    </div>
                </div>
            ):(
